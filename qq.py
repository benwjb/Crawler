...
def getLinks(url):
    ...
    url_list=Selector.xpath('//p[@class="first"]/a/@href')
    for i in range(len(url_list)):
        url_list[i]='http://www.qcsp3.com'+url_list[i]
        #print(url_list[i])
    return url_list
...
def getImg(url,i):
    html=getHTML(url)
    Selector=etree.HTML(html)
    img_list=Selector.xpath('//div[@class="right-cont-img"]/img/@src')
    t_list=Selector.xpath('//p[@class="title"]/text()')
...
def downloadimg1(url_list,num):
	if not os.path.exists('D://downloqq'):
		os.mkdir('D://downloqq')
	rootpath=os.getcwd()
	for i in range(num):
		imgdir='D://downloqq/'#+url_list[i][25:-9].replace("/", '')
...
def downloadimg(url_list,num):
	...
	rootpath=os.getcwd()
	for i in range(num):
		imgdir='D://downloqq/'#+url_list[i][25:-5].replace("/", '')
		if not os.path.exists(imgdir):
			os.mkdir(imgdir)
		os.chdir(imgdir)
		getImg(url_list[i],i)
...
if __name__ == '__main__':
    ...
    #print("--------------------请输入地址：")
    #url= input('')
    url="http://www.qcsp3.com/pictures/index_page_.html" 
while True:
    print("--------------------请输入页数：")
    page=input("")
