...
def getLinks(url):
    ...
    url_list=Selector.xpath('//div[@class="thumbmm"]/a/@href')
...
def getImg(url,i):
    html=getHTML(url)
    Selector=etree.HTML(html)
    img_list=Selector.xpath('//div[@class="entry"]/a/p/img/@src')
    t_list=Selector.xpath('//title/text()')
    url_list2=Selector.xpath('//ul[@class="dm-fy"]/li/a/@href')
    print(len(img_list))
    #print(img_list)
    ...
    try:
        ...
        #fp=open('%s_%d_%d.jpg' % (No,picN,id),'wb+') #打开一个文本文件
            ...
            print('%s_%d.jpg' %(pic,id)+" "+"is finished")
            id +=1 #写入数据
            fp.close()
    #pic_name = 0
    #for each in img_list:
...
def downloadimg1(url_list,num):
	if not os.path.exists('downloadtuao82'):
		os.mkdir('downloadtuao82')
	rootpath=os.getcwd()
	for i in range(num):
		imgdir='downloadtuao82/'#+url_list[i][44:-6].replace("/", '')
...
if __name__ == '__main__':
    ...
    #print("--------------------请输入地址：")
    #url= input('')
    url="https://www.tuao8.top/category-2.html" 
while True:
    print("--------------------请输入页数：")
    page=input("")
