...
def getLinks(url):
	...
	url_list=Selector.xpath('//div[@class="box list channel"]/ul//li/a/@href')
	for i in range(len(url_list)):
		url_list[i]='https://www.997cf.com'+url_list[i]
		#print(url_list[i])
	return url_list
...
def downloadimg(url_list,num):
	if not os.path.exists('downloadasian'):
		os.mkdir('downloadasian')
	rootpath=os.getcwd()
	for i in range(num):
		imgdir='downloadasian/'+url_list[i][31:-4].replace(".", '')
...
if __name__ == '__main__':
	...
	#print("--------------------请输入地址：")
	#url= input('')
	url="https://www.997cf.com/htm/piclist2/" 
while True:
	print("--------------------inputpagenums:")
	page=input("")
