...
def getLinks(url):
	...
	url_list=Selector.xpath('//th[@class="subject common"]/span/a/@href')
...
def downloadimg(url_list,num):
	if not os.path.exists('E://downloadss'):
		os.mkdir('E://downloadss')
	rootpath=os.getcwd()
	for i in range(num):
		imgdir='E://downloadss/'+url_list[i][40:47].replace("&", '')
...
if __name__ == '__main__':
	...
	#print("--------------------请输入地址：")
	#url= input('')
	url="http://h.91p11.space/forumdisplay.php?fid=21&filter=digest&page=" 
while True:
	print("--------------------请输入页数：")
	page=input("")
