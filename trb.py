...
def getLinks(url):
    ...
    for i in range(len(url_list)):
        url_list[i]='http://52zfl.com/'+url_list[i]
        #print(url_list[i])
    return url_list
...
def downloadimg1(url_list,num):
	if not os.path.exists('D://downlo4'):
		os.mkdir('D://downlo4')
	rootpath=os.getcwd()
	for i in range(num):
		imgdir='D://downlo4/'#+url_list[i][25:-9].replace("/", '')
...
def downloadimg(url_list,num):
	...
	rootpath=os.getcwd()
	for i in range(num):
		imgdir='D://downlo4/'#+url_list[i][25:-5].replace("/", '')
		if not os.path.exists(imgdir):
			os.mkdir(imgdir)
		os.chdir(imgdir)
		getImg(url_list[i],i)
...
if __name__ == '__main__':
    ...
    #print("--------------------请输入地址：")
    #url= input('')
    url="http://52zfl.com/luyilu/list_5_1.html" 
while True:
    print("--------------------请输入页数：")
    page=input("")
