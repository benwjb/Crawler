...
def getLinks(url):
    ...
    url_list=Selector.xpath('//a[@class="focus"]/@href')
    #for i in range(len(url_list)):
        #url_list[i]='http://bbt01.net/'+url_list[i]
        #print(url_list[i])
    return url_list
...
def getImg(url,i):
    html=getHTML(url)
    Selector=etree.HTML(html)
    img_list=Selector.xpath('//article[@class="article-content"]/p/img/@src')
    t_list=Selector.xpath('//h1[@class="article-title"]/a/text()')
    url_list2=Selector.xpath('//div[@class="article-paging"]/a/@href')
    #print(len(img_list))
    #print(img_list)
    ...
    try:
        for each in img_list:
            ...
            #print("do")
            print('%d_%d.gif' %(pic_name,picN)+" "+"is finished")
            pic_name += 1
            time.sleep(2) #关闭文件
    except: #Exception:
        pass 
...
def downloadimg1(url_list,num):
	if not os.path.exists('D://downlord'):
		os.mkdir('D://downlord')
	rootpath=os.getcwd()
	for i in range(num):
		imgdir='D://downlord/'#+url_list[i][25:-9].replace("/", '')
...
def downloadimg(url_list,num):
	...
	rootpath=os.getcwd()
	for i in range(num):
		imgdir='D://downlord/'#+url_list[i][25:-5].replace("/", '')
		if not os.path.exists(imgdir):
			os.mkdir(imgdir)
		os.chdir(imgdir)
		getImg(url_list[i],i)
