...
def getLinks(url):
	...
	url_list=Selector.xpath('//a[@class="z"]/@href')
	for i in range(len(url_list)):
		url_list[i]='http://108.170.9.194/'+url_list[i]
		#print(url_list[i])
	return url_list
...
def getImg(url,i):
	...
	t_list=Selector.xpath('//a[@id="thread_subject]/text()')
	#print(len(img_list))
	#print(img_list)
	for i in range(len(img_list)):
		img_list[i]='http://108.170.9.194/'+img_list[i]
		#print(img_list[i])
	No=url[44:-13]
	#picN=i
...
def downloadimg(url_list,num):
	if not os.path.exists('E://downloadx'):
		os.mkdir('E://downloadx')
	rootpath=os.getcwd()
	for i in range(num):
		imgdir='E://downloadx/'+url_list[i][45:61].replace("/", '')
...
if __name__ == '__main__':
	...
	#print("--------------------请输入地址：")
	#url= input('')
	url="http://108.170.9.194/forum-762-" 
while True:
	print("--------------------请输入页数：")
	page=input("")
